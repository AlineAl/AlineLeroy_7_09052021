<template>
    <div class="container">
        <div class="navbar">
            <div>
               <img src="../assets/images/icon-left-font-monochrome-black.png" alt="groupomania" class="logo"> 
            </div>
            <div class="list">
                <div>
                   <ul>
                        <li class='list-navbar'><i class="fas fa-power-off"></i></li>
                        <li class='list-navbar'><i class="fas fa-list"></i></li>
                    </ul> 
                </div>
                <div>
                   <img src="../assets/images/—Pngtree—vector users icon_4144740.png" alt="photo de profil" class="avatar"> 
                </div>
            </div>
        </div>

        <div class="card-login">
            <form action="" method="post">
                <img src="../assets/images/icon-left-font-monochrome-black.png" alt="">
                <div>
                    <input class="form-email-password" type="text" id="nom" name="nom" placeholder="Nom" required>  
                </div>
                <div>
                    <input class="form-email-password" type="text" id="prénom" name="prénom" placeholder="Prénom" required>  
                </div>
                <div>
                    <input class="form-email-password" type="text" id="email" name="email" placeholder="Email" required>  
                </div>
                <div>
                    <input class="form-email-password" type="text" id="password" name="password" placeholder="Mot de passe" required> 
                </div>
                <div>
                    <input class="form-email-password" type="text" id="poste" name="poste" placeholder="Poste occupé" required> 
                </div>
                <div class="button-form">
                    <input class="button" type="submit" value="S'inscrire">
                </div>
            </form>
        </div>
    </div>
</template>

<script>

export default { }

</script>